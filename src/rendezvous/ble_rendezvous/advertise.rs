...
pub async fn advertise_rendezvous() -> Result<(), Box<dyn Error>> {
    ...
    println!("Encoded Network payload (IP: {:?}, Port: {}) -> Name: '{}'", ip, port, device_name_payload);
    ...
    peripheral.start_advertising(&device_name_payload, &[service_uuid]).await?;
    println!("Now actively advertising custom network rendezvous info...");

    // 5. Keep advertising forever (@todo: in future versions we will adapt the advertise duration)
    loop {
